...
#
# Stages 3 and 4 ask about things outside the checkout, so someone who installed
# this as a PopTracker pack sees two skips and a green run rather than a wall of
# failures. Override any of these to point it elsewhere:
#
...
if "$ROOT/tests/run.sh" >/dev/null 2>&1; then
...
if [ ! -d "$CORPUS" ]; then
    ...
    logic_fail=0
    # check_logic exits 0 whether or not it found a divergence, so an exit code
    # cannot gate this stage -- and piping it into `tail` would have gated on
    # `tail`. The count is the signal, and it is compared against what the
    ...
    for pair in "nov/oracle_nov 1" "std/oracle_std 0"; do
        ...
        # Exit status is not the gate. check_logic exits 1 when it finds a
        # divergence, and `nov` is expected to find one, so a non-zero exit is
        # the normal case on that cartridge. The count is what is compared;
