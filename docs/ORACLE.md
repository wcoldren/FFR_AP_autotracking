...
| the incentive rings, all 23 exports | **0 wrong**, since 2026-09-03. `tools/tests/test_incentive_conjunction.py` predicts each slot's ring from the export's own flags and compares it to `priority_locations`. Before the conjunctions it was 13 wrong rings and one ghost -- 7 on `nofetchitems497`, 6 plus the caravan slot on `nonpcitems497` -- and before Nerrick's third term, `nov` and `nov2` each rang him. Three disagreements are waived by name rather than scoped away: Cardia on the six hoard cartridges, a Sea Shrine chest `notail` does not have, and `Dr Unne`, who is no seed's location |
