...
| `Entrances`, `Floors`, `Towns`, `EntrancesMixedWithTowns`, `IncludeConeria`, `AllowDeepCastles` | `Entrances`, `Towns` and `Floors` derive `entranceShuffle` | unmodellable by toggle; `regen_maps` reads the result off the cartridge. Read since 2026-09-04, but for a display rather than a rule: `applyFFRFlagsToBoard` ORs the first three into the ungridded `entranceShuffle` item, which is what the Auto stage of `Entrance Pins` asks before drawing the door markers. An item and not an `ffrFlag` call, because PopTracker re-asks a marker's visibility rule when an item changes and at no other time. None of them gains a cell or a rule, so the status is unchanged |
