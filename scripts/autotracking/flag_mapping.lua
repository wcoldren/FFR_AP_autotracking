...
local NOT_MODELLED = {
  ...
  },
  {
    ...
    why = "walked on a No-Overworld pair one flag apart, 2026-09-03, and it "
      .. "moves no link and no rule. It does edit the map: it chooses "
      .. "between two sets of tile writes to MapIndex.Lefein in ApplyMapMods "
      .. "(MetroidVaniaMap.cs:58, No-Overworld only), and it places the "
      .. "store itself from Update() on either mode "
      .. "(StandardMaps/SMUpdates.cs:39 at 4.9.2), ANDed with the "
      .. "ShopKillMode pair FlagsCompute.cs:51 names LefeinSuperStoreEnabled. "
      .. "But the two branches each leave exactly one two-cell plug in "
      .. "Lefein's left-edge column, one row apart -- on, the plug is at "
      .. "y=1 and (0x00,0x02) is walkable; off, the plug is at y=2 and "
      .. "(0x00,0x01) is walkable -- so nothing opens and nothing closes. "
      .. "Both cartridges derive 256 locations with the same names, the "
      .. "entrance graph is identical apart from the gateway permutation "
      .. "and the ToFR bonus chests that every No-Overworld seed rolls, and "
      .. "check_logic grades the flag-off cartridge with the same single "
      .. "strict Lefein divergence as nov. Held as noise until 2026-09-01 on "
      .. "the word 'store' in the name, then unjudged on a call site cited "
      .. "without being read. docs/ORACLE.md, 'LefeinSuperStore off: what it "
      .. "moves, which is not a link', has the figures.",
  },
}
